...
The goal of `BatchCorrection` is to check if there are possible batch effects in the data and serveral
batch correction methods are available to apply if needed.

Batch correction methods available:

- **median correction:** With the median batch correction method a correction factor for each feature is calculate by dividing the overall median by the median per batch for all features in the pooled samples. This correction factor is then used to correct all samples.
- **LOESS:** The LOESS batch correction method [1] depends on pooled samples being measured in regular intervals. It can correct signal intensity
...
- **ComBat (SVA):** The sva package [2] contains functions for removing batch effects and other un-
...
2. Leek JT, Johnson WE, Parker HS, Jaffe AE, and Storey JD. (2012) 
